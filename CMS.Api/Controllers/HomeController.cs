...
namespace CMS.Api.Controllers
{
    ...
    public class HomeController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<Result<HomeResponseDto>> Post([FromForm] HomeRequestDto request, CancellationToken cancellationToken)
        {
            ...
            var imageUrl = await _service.UploadImage(request, request.image);

            var logoUrl = await _service.UploadImage(request, request.logo);

            var mappedCommand = await command.FromRequest(request);

            mappedCommand.ImageUrl = imageUrl.Data;
            mappedCommand.LogoUrl = logoUrl.Data;
        ...
        public async Task<Result<HomeResponseDto>> Put([FromRoute] int id, [FromForm] HomeRequestDto request, CancellationToken cancellationToken)
        {
            ...
            if (request.image is not null && request.logo is not null)
            {
                ...
                var uploadLogo = await _service.UploadImage(request, request.logo);
        ...
        [HttpGet("arabic")]
        public async Task<Result<IEnumerable<HomeResponseDto>>> GetArabicHostingPackages(CancellationToken cancellationToken)
        {
            var query = new GetHomeBasedonLangQuery { lang = "ar" };
            var response = await _sender.Send(query, cancellationToken);

            if (response.IsSuccess)
            {
                return await Result<IEnumerable<HomeResponseDto>>.SuccessAsync(response.Data, ResponseStatus.GetAllSuccess, true);
            }
        ...
        [HttpGet("english")]
        public async Task<Result<IEnumerable<HomeResponseDto>>> GetEnglishHostingPackages(CancellationToken cancellationToken)
        {
            var query = new GetHomeBasedonLangQuery { lang = "en" };
            var response = await _sender.Send(query, cancellationToken);
