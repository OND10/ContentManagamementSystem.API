...
namespace CMS.Infustracture.Implementation
{
    public class HomeRepository : IHomeRepository
    {
        ...
        public async Task<IEnumerable<Home>> GetHomeByLangAsync(string lang, CancellationToken cancellationToken)
        {
            using (IDbConnection db = _context.CreateConnection())
            {
                var query = "SELECT * FROM dbo.Get_Home_by_lang(@lang)";
                var result = await db.QueryAsync<Home>(query, new { lang });
