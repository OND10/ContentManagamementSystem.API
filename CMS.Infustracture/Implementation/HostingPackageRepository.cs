...
namespace CMS.Infustracture.Implementation
{
    public class HostingPackageRepository : IHostingPackageRepository
    {
        ...
        public async Task<PackageReservation> ReservePackageAsync(PackageReservation entity, CancellationToken cancellationToken)
        {

            using (IDbConnection db = _context.CreateConnection())
            {
                    var sql = @"
                INSERT INTO PackageReservation (
                    UserEmail, FullName, PhoneNumber, CompanyName, StreetNo, 
                    City, Country, PackageName, PackageDescription, PackageCategory, 
                    MonthlyPrice, YearlyPrice, CpuCapacity, HardCapacity, TypeofHard
                ) 
                VALUES (
                    @UserEmail, @FullName, @PhoneNumber, @CompanyName, @StreetNo, 
                    @City, @Country, @PackageName, @PackageDescription, @PackageCategory, 
